...
function animalsByIds(...ids) {
  ...
  ids.map((id) => {
    return arr.push(animals.find(animal => animal.id === id));
  });
  return arr;
}
...
function animalsOlderThan(animal, age) {
  // seu código aqui
  let animalName = animals.find(animalName => animalName.name === animal);
  let checkAge = animalName.residents.every(residentAge => age < residentAge.age);
  return checkAge;
}
...
function animalCount(species) {
  ...
  if(species != undefined){
    let animalsName = animals.find( animal => animal.name == species)
    return animalsName.residents.length
  }else{
    let animalCount = animals.reduce( (acc, {name, residents}) => {
