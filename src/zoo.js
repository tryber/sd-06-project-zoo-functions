...
function searchAnimalsForEmployee(idOrName) {
  ...
  result[`${dataEmployee.firstName} ${dataEmployee.lastName}`] = dataEmployee.responsibleFor
    .map(animalListId => animals
      .find(findAnimalsId => findAnimalsId.id === animalListId).name);
  return result;
}
